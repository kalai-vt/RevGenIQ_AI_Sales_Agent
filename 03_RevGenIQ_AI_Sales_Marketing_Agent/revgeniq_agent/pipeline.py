...
async def run_pipeline(
    ...
    try:
        ...
        _TOPICAL_INTENTS = ("pricing", "product_inquiry", "support", "complaint", "off_topic", "greeting", "farewell")
        is_mid_capture = (
        ...
        if is_mid_capture:
        ...
        elif intent in ("support", "complaint"):
            agent = SupportAgent(db)
            structured = await agent.respond(message, tenant_id, company_context, effective_history, intent)
            ...
            agent = RAGAgent(db)
            structured = await agent.respond(message, tenant_id, company_context, effective_history)
